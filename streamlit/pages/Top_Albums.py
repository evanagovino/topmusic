...
with st.sidebar:
    ...
    if year_selection == 'One Year':
        show_years = st.selectbox('Year', list(range(1959, datetime.datetime.now().year + 1))[::-1], index=0)
        min_year = show_years
        max_year = show_years
        ...
        col1, col2 = st.columns(2)
        with col1:
            min_year = st.selectbox('First Year', range(1959, datetime.datetime.now().year + 1), index=0)
        with col2:
            max_year = st.selectbox('Last Year', list(range(1959, datetime.datetime.now().year + 1))[::-1], index=0)
